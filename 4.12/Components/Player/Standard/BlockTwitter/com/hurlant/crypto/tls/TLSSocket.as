	...
	public class TLSSocket extends EventDispatcher implements IDataInput, IDataOutput {
		...
		public function dispose():void
		{
			if (_engine!=null)
			{
				...
				_engine.removeEventListener(ProgressEvent.SOCKET_DATA, onEngineProgress);
				try
				{			
					_engine.close();
				}
			...
			if (_socket!=null)
			{
				...
				_socket.removeEventListener(ProgressEvent.SOCKET_DATA, onSocketProgress);
				try
				{			
					_socket.close();
				}
		...
		public function startTLS(socket:Socket, host:String, config:TLSConfig = null):void 
		{
			if (!socket.connected) 
			{
				...
				trace("Cannot STARTTLS on a socket that isn't connected.");
			}
			else
			{
		...
		private function init(socket:Socket, config:TLSConfig, host:String):void {
			...
			_engine.addEventListener(ProgressEvent.SOCKET_DATA, onEngineProgress);
			_socket.addEventListener(ProgressEvent.SOCKET_DATA, onSocketProgress);

			_ready = false;
		}
		
		
		private function onEngineProgress(event:ProgressEvent):void
		{
			_socket.flush();
		}

		private function onSocketProgress(event:ProgressEvent):void
		{
			_engine.dataAvailable(event);
		}
